...
def run_sensitivity_study(write_to, directory, truth_path, truth_U_inlet, truth_alpha, loaded):
    ...
    fig.suptitle(f'Means wrt. number of drones: U={truth_U_inlet}_a={truth_alpha}')

    axes[0].plot(results[:,0], results[:, 1], label='randomly within zone')
    axes[0].plot(results_optimized[:,0], results_optimized[:, 1], label='optimized selection')
    axes[0].plot(results_lines[:,0], results_lines[:, 1], label='line selection')
    axes[0].set_yscale('log')
    axes[0].set_xlabel('Number of drones')
    axes[0].set_ylabel('Mean Initial Velocity STD')
    axes[0].set_title('Inlet velocity')

    axes[1].plot(results[:,0], results[:, 3], label='randomly within zone')
    axes[1].plot(results_optimized[:,0], results_optimized[:, 3], label='optimized selection')
    axes[1].plot(results_lines[:,0], results_lines[:, 3], label='line selection')
